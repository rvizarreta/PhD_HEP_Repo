...
def main(filename=None, ttree_filename=None, file_mode="RECREATE", tree_name="mwpc"):
    if filename is None:
    ...
    if ttree_filename is None:
        ttree_filename = "myTest.root"
        
    logging.info("Processing %s into %s using %s mode." % (filename, ttree_filename, file_mode))
    el = eventloop.EventLoop(ttree_filename, filename, file_mode, tree_name)
