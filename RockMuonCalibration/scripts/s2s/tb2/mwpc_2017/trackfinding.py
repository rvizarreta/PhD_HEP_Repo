...
class Module(object):
    ...
    def conservativecut(self):
        # cut if there is more than 1 poi t
        if self.getnpoints() > 1:
            print "Cutting", self.getnpoints(), "points in module", self.module_number
            self.removepoints()
...
def findlines(modules, mingoodness=None, conservative=False):
    ...
    length = len(modules)
    if conservative:
        assert length == 4 # TODO is this always the case
    ...
    while length >= 3:
        ...
        for combination in usedmodules:
            ...
            #exit()
            foobar = list(itertools.product(*map(lambda x: x.points, combination)))
            # sanity check
            if conservative and length == 4:
                # foobar is a list of all the combos of points
                # so a list of list of points
                # in conservative mode there should only be 1 set of points
                assert len(foobar) < 2
            for points in foobar:
                # sanity checks
                ...
                if line is None: 
                    if conservative:
                        print "WARNING: Was unable to make a line in conservative mode"
                    # skip, fit didn't work
                    continue 
                    
                gof = line.get_goodness_of_fit()
                # check if line is good, 
                # or if conservative mode, allow all lines
                if gof <= mingoodness or conservative:
        ...
        if len(potential_lines) == 0:
            ...
            length -= 1
            if conservative:
                print "WARNING: Trying to change length to length - 1 in conservative mode. Length =", length
            continue 
...
def maketracksfromclusters(clustersbymodule, mingoodness=None, conservative=False):
    n = 4
    ...
    if conservative:
        for module in itertools.chain(modulesx, modulesy):
            module.conservativecut()
                
    tracksx = findlines(modulesx, mingoodness, conservative)
    tracksy = findlines(modulesy, mingoodness, conservative)
    ...
    tracks = maketracksmoreaccurate(tracks)
...
def getsigma(line, pos):
    assert len(line.points) == 2
    dy1 = line.points[0].dy
    dy2 = line.points[1].dy
    x1 = min(line.points[0].x, line.points[1].x)
    x2 = max(line.points[0].x, line.points[1].x)
    assert x2 != x1
    # TODO this is an estimate assuming triangles
    return abs((dy1 + dy2) * (pos - x1) / float(x2 - x1))
    
def calcavg(*args):
    out = 0.0
    wgts = 0.0
    for var, sigma in args:
        sigma = float(sigma)
        out += (var / sigma**2) if sigma != 0 else 0
        wgts += (1 / sigma**2) if sigma != 0 else 0
    if wgts != 0:
        out /= float(wgts)
        return out
    else: assert wgts != 0 # shouldn't happen
    
def maketracksmoreaccurate(tracks):
    out = []
    for track in tracks:
        foo = dict()
        for p in track.points:
            wc = p.getWC()
            foo[wc] = p
        listOfPermutations = itertools.combinations(foo.items(), 2)
        bar = dict()
        for permutation in listOfPermutations:
            listofpoints = map(lambda x: x[1], permutation)
            wcCombo = tuple(map(lambda x: x[0], permutation))
            line = makelinefrompoints(listofpoints)
            bar[wcCombo] = line
        if (1, 4) in bar and (2, 3) in bar:
            #print "Not missing 14, 23"
            # combine the line
            l14 = bar[(1, 4)]
            l23 = bar[(2, 3)]
            # get the detector position's Z value
            detpos = calibration.instrument_positions[1][0]
            sigma14 = getsigma(l14, detpos)
            sigma23 = getsigma(l23, detpos)
            m = calcavg((l14.m, sigma14), (l23.m, sigma23))
            b = calcavg((l14.b, sigma14), (l23.b, sigma23))
            # TODO calc dm, chi2, db and so on
            # as a quick thing, I set chi2 to -2 for easy finding
            line = Line(m = m, b = b, dm = track.dm, db = track.db, chi2 = -2, points = track.points)
            out.append(line)
        else: 
            #print "maketracksmoreaccurate: Missing 1,4 and 2,3 in bar"
            # add the track
            out.append(track)
    return out
