...
FINALS2S = False # TODO change to true when ready
SAVEOLDERROR = False
TRACKER = True

# Functionality to skip specific module/strip combinations
# like if you're getting a 
# gsl error: integral is divergent or slowly convergent.
# (module, strip)
# Make sure to remove these strips each iteration to
# see if the problem fixed itself
POSITIONS_TO_SKIP = [(3, 62)]


# README:
# The output S2S constants, the ones you want to use, are in,
# energy_plots/s2s_constants.txt
...
def printfixeds2s(c, s2s):
    ...
    TEST_THESE_MODULES = (0, 1, 10, 20, 40, 41)
...
def getmoduleavgs2s(d, module):
    out = 0
    n = 0
    out2 = dict()
    for strip in range(64):
        if (module, strip) not in d: 
            out2[strip] = 1.0
            continue
        info = d[(module, strip)]
        s2s, err = info[0], info[5]
        out2[strip] = s2s
        if err > 0: continue # skip error
        out += s2s
        n += 1
...
def gets2s(c):
    ...
    TEST_THESE_MODULES = ()#(1, 20, 29)# (0, 1, 20, 29, 40, 41)
    ...
    for iteration in range(NITERATIONS):
        for module in range(0, 42): # module goes from 0 to 41
            ...
            nbins = getnbins(module)
            # Multiply by module avg excluding error strips
            moduleavgs2s, factors = getmoduleavgs2s(d, module) if iteration != 0 else (1, None)
            moduleavgs2s = 1.0 # We don't need this anymore since we have the factors, no double counting
            if factors != None:
                text = []
                for strip, s2s in factors.items():
                    text.append("(({1}*(st_strip == {0}))+(st_strip != {0}))".format(strip, s2s))
                text = "*".join(text)
            else: text = "1.0"
            print "Avg module S2S for module %s:" % module, moduleavgs2s
            print "Text:", text
            c.Draw("st_strip:%s*(%s)*10.0*st_mev/st_path>>htemp(%s, 0, 10, 64, -0.5, 64 - 0.5)" % (moduleavgs2s, text, nbins), 
            ...
            try: os.mkdir("energy_plots/module{0:02d}".format(module))
            except OSError: pass # already exists
            for strip in range(1, 64): # strip goes from 1 to 63
                ...
                if temp.GetEntries() == 0 or pos in POSITIONS_TO_SKIP: 
                    if temp.GetEntries() == 0: print "Zero entries, skipping module", module, "strip", strip
                    else: print "Setting default value for module", module, "strip", strip, "because it's in POSITIONS_TO_SKIP"
                    info = (1.0, 0, 0, 0, 0, 100)
                    d[pos] = info
                    continue
                    
                #if pos == (33, 15):  # TODO temp, skill due to gsl: qags.c:563: ERROR: integral is divergent, or slowly convergent OR Default GSL error handler invoked.
                #    info = (1.0, 0, 0, 0, 0, 100)
                #    d[pos] = info
                #    continue
                ...
                if ndf == 0: 
                    ...
                    d[pos] = info
                    continue
                s2s = 2/x if x != 0 else 1.0
                if factors != None:
                    s2s *= moduleavgs2s * factors[strip] # Multiply by const from above
                else: s2s *= moduleavgs2s
                ...
                if errcode != 0: # or module == 41: # TODO add back if plane 41 has problems.
                    err += 1
                ...
                temp.Draw()
                c1.Print("energy_plots/module{0:02d}/strip{1:02d}_iter{2:02d}.png".format(module, strip, iteration))
                if err != 0:
                    temp.Draw()
                    c1.Print("energy_plots/errors/module{0:02d}_strip{1:02d}_iter{2:02d}.png".format(module, strip, iteration))
...
def getheader(c):
    ...
    maxtime = -mintime
    c.SetBranchStatus("*", 0)
    c.SetBranchStatus("begin_gps_time", 1)
    c.SetBranchStatus("end_gps_time", 1)
    for event in c:
        if event.begin_gps_time < 1000: continue
        mintime = min(mintime, event.begin_gps_time)
        maxtime = max(maxtime, event.end_gps_time)
    out = """# begin   {0} Mon Apr 06 18:29:55 2015
# end     {1} Tue Apr 21 07:38:31 2015
# elapsed {2} min
# min occ 1 hits
# detector subdet module plane strip s2s ds2s entries error
""".format(mintime, maxtime, maxtime-mintime)
    print "S2S File Header:"
    print out
    c.SetBranchStatus("*", 1)
    return out
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        olds2s = sys.argv[2]
    else: print "Warning: Didn't specify an old S2S"
    ...
    try: os.mkdir("energy_plots/errors")
    except OSError: pass # already exists
    try: os.mkdir("energy_plots/fixed")
    except OSError: pass # already exists
    ...
    for pos, info in d.items():
        s2s, s2serr, n, chi2, chi2N, err = info
        s2s *= const
        finals2s[pos] = (s2s, s2serr, n, chi2, chi2N, err)
        news2s[pos] = (news2s[pos][0] * const, s2serr, n, chi2, chi2N, err)
        
    # Finally write the output
    # THIS IS THE FILE THAT'S THE OLD S2S CONSTS * NEW CONSTS
    with open("energy_plots/s2s_constants.txt", "w") as f:
        f.write(header)    
        # Write info on each strip
        for pos, info in sorted(finals2s.items()):
            l = list(pos)
            l.extend(info)
            module, strip = pos
            # module 21 is first HCal module
            subdet = 3 if TRACKER else 4 
            l.append(subdet if module < 21 else 5) 
    ...
    # THIS CONTAINS ONLY THE CHANGE IN S2S. USE ABOVE TO RERUN COSMICS
    with open("energy_plots/s2s_constants_new.txt", "w") as f:
        f.write(header)    
        # Write info on each strip
        for pos, info in sorted(news2s.items()):
            l = list(pos)
            l.extend(info)
            module, strip = pos
            # module 21 is first HCal module
            l.append(4 if module < 21 else 5) # TODO change 3 to 4 for ecal, 3 for tracker
            # TODO add plane and detector info if needed
            idk = "0 {8} {0} 1 {1} {2} {3} {4:d} {7}\n".format(*l)
            f.write(idk)
    #exit() # TODO remove
    printfixeds2s(c, news2s)
