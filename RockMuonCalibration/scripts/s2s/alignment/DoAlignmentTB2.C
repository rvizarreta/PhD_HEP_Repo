...
void DoAlignmentTB2()
{
  ...
  TProfile3D * p3ModLposBase = (TProfile3D *)fin->Get("moduleLposBase");
  TProfile3D * p3ModTposBase = (TProfile3D *)fin->Get("moduleTposBase");
  cout<<p3ModLposBase<<", "<<p3ModLposBase->GetEntries()<<", "<<p3ModLposBase->Project3D("yz")<<endl;

  p3ModLposBase->GetYaxis()->SetTitle("lpos");
  p3ModLposBase->GetZaxis()->SetTitle("triangle base");

  p3ModTposBase->GetYaxis()->SetTitle("tpos");
  p3ModTposBase->GetZaxis()->SetTitle("triangle base");
  ...
  for (int modbin = 1; modbin <= 240; ++modbin)
  {
    ...
    p3ModTposBase->GetXaxis()->SetRange(modbin, modbin);

    char H2Name[40];
    sprintf(H2Name, "%s_2D", ModTitle);
    TH2 * h2BaseLpos = (TH2 *)p3ModLposBase->Project3D("yz");
    TH2 * h2BaseTpos = (TH2 *)p3ModTposBase->Project3D("yz");
    h2BaseLpos->SetName(H2Name);
    h2BaseTpos->SetName(H2Name);
    h2BaseLpos->SetTitle(Form("Module %d Plane %d;Base Position (mm);Longitudinal Position (mm);Average Energy (MeV)",module,plane));
    h2BaseTpos->SetTitle(Form("Module %d Plane %d;Base Position (mm);Transverse Position (strip num);Average Energy (MeV)",module,plane));
    ...
    for (int i = 0; i < 6; ++i)
    {
      ...
      float dy = 480;
      lpos_point[i] = -dy + (2*dy/float(6) * i) + dy/float(6);
    ...
    c2->cd(0);
    //gStyle->SetOptStats(false);
    h2BaseTpos->SetStats(false);
    h2BaseTpos->Draw("colz");
    c2->Print(Form("alignmentPlotDump/mod%03dpl%d_tpos.png",module,plane));
    //gStyle->SetOptStats(true);
    delete c2;
